...
var instructions1 = [
  ...
  "Let's practice! On the next screen, you'll have nine pulls. Use the F key to sample the left slot machine for any one of those 9 pulls, or the J key to sample the right slot machine.",
...
var instructions2c = [
  ...
  "After those four forced choices, you will have either 1 or 5 lever pulls left, for which you are free to choose however you wish.",
...
function instructions4(){
  return[
    ...
    "That's all there is to it! Each one of these iterations of 5 or 9 machine pulls we can call a 'game'. Press next to practice a game of each length before moving on to the main task."
  ].map(x => createInstrPage(x))}

function instructions5(){
  return[
    "You just practiced a game with 5 pulls, press next to practice a game with 9 pulls."
  ].map(x => createInstrPage(x))}
